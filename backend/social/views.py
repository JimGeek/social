...
# Sentry Logging and Monitoring Views

class SentryLogsView(APIView):
    """Sentry error monitoring and logging endpoint"""
    permission_classes = [permissions.AllowAny]  # Allow public access for monitoring
    
    def get(self, request):
        """Get application health and error status"""
        try:
            ...
            health_status = {
                ...
                'version': getattr(settings, 'SENTRY_RELEASE', 'unknown'),
            ...
            # Get recent error count (if available)
            try:
                # This is a placeholder - in production you might query a logs table
    ...
    def post(self, request):
        """Log custom events and errors to Sentry"""
        try:
            ...
            return Response({
                'success': True,
                'message': 'Event logged to Sentry',
                'type': event_type,
                'timestamp': timezone.now().isoformat()
            })
...
class SentryTestView(APIView):
    ...
    def post(self, request):
        """Generate test errors for Sentry monitoring"""
        try:
            test_type = request.data.get('type', 'info')
            
            if test_type == 'error':
                # Generate a test error
                raise Exception("This is a test error for Sentry monitoring")
            
            elif test_type == 'performance':
                # Generate a slow operation for performance monitoring
                import time
                start_time = time.time()
                time.sleep(2)  # Simulate slow operation
                end_time = time.time()
                
                capture_message(f"Performance test completed in {end_time - start_time:.2f} seconds", level='info')
                ...
                with sentry_sdk.configure_scope() as scope:
                    ...
                    scope.set_tag("test_type", "user_context")
                
                capture_message("User context test message", level='info')
                ...
                capture_message("Sentry integration test message", level='info')
    ...
    def get(self, request):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute("SELECT 1")
                connection_count = 1  # Simple connection test
            ...
            metrics = {
                ...
                'application': {
                    ...
                    'active_accounts': SocialAccount.objects.filter(status='connected').count(),
                }
            }

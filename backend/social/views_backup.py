...
class SocialPostViewSet(viewsets.ModelViewSet):
    ...
    def publish(self, request, pk=None):
        ...
        # Validate that accounts belong to the organization
...
class SocialPostTargetViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SocialPostTarget.objects.filter(
            post__organization=self.request.user.organization
        )
...
class SocialAnalyticsViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return SocialAnalytics.objects.filter(
            post_target__post__organization=self.request.user.organization
        )
...
class SocialCommentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SocialComment.objects.filter(organization=self.request.user.organization)
...
class SocialIdeaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SocialIdea.objects.filter(organization=self.request.user.organization)
...
class SocialHashtagViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SocialHashtag.objects.filter(organization=self.request.user.organization)
    ...
    def suggestions(self, request):
        ...
        try:
            ...
            # Get popular hashtags from organization
...
class SocialQueueViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SocialQueue.objects.filter(organization=self.request.user.organization)
...
class SocialMediaFileViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return SocialMediaFile.objects.filter(organization=self.request.user.organization)
...
class FacebookConnectView(APIView):
    ...
    def get(self, request):
        ...
        state_data = {
            ...
            'org_id': str(request.user.organization.id) if request.user.organization else None
        }
        ...
        auth_url = f"{settings.FACEBOOK_OAUTH_URL}?{urlencode(params)}"
        ...
        request.session['oauth_organization_id'] = str(request.user.organization.id) if request.user.organization else None
...
class FacebookCallbackView(APIView):
    ...
    def get(self, request):
        ...
        if not user:
            ...
            request.session.pop('oauth_organization_id', None)
            request.session.pop('fb_oauth_state', None)
            return redirect(f"{settings.FRONTEND_URL}/login?error=authentication_required&message=Session expired during OAuth flow")
        
        if error:
            return redirect(f"{settings.FRONTEND_URL}/{org_slug}/social/settings?error=facebook_oauth_error&message={error}")
        ...
        if not code:
            return redirect(f"{settings.FRONTEND_URL}/{org_slug}/social/settings?error=no_authorization_code")
        ...
        try:
            ...
            if not token_data['success']:
                return redirect(f"{settings.FRONTEND_URL}/{org_slug}/social/settings?error=token_exchange_failed&message={token_data['error']}")
            ...
            return redirect(f"{settings.FRONTEND_URL}/{org_slug}/social/settings?success=facebook_connected&accounts={accounts_count}&token={token}")
            
        except Exception as e:
            return redirect(f"{settings.FRONTEND_URL}/{org_slug}/social/settings?error=connection_failed&message={str(e)}")
...
class InstagramCallbackView(APIView):
    ...
    def _create_or_update_account(self, platform_name, account_data, access_token, expires_at, request, user=None):
        ...
        try:
            ...
            print(f"Organization: {actual_user.organization if hasattr(actual_user, 'organization') else 'No organization'}")
            
            platform = SocialPlatform.objects.get(name=platform_name)
            print(f"Found platform: {platform}")
            
            organization = actual_user.organization if hasattr(actual_user, 'organization') else None
            if not organization:
    ...
    def get(self, request):
        ...
        try:
            ...
            analytics_qs = SocialAnalytics.objects.filter(
                post_target__account__organization=organization,
...
class SyncAnalyticsView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Perform sync
            sync_results = analytics_service.sync_all_analytics(str(organization.id), days_back)
...
class ExportAnalyticsView(APIView):
    ...
    def _export_analytics(self, request):
        ...
        try:
            ...
            # Generate filename
            filename = f'social_analytics_{organization.name}_{datetime.now().strftime("%Y%m%d")}'
...
class LiveDataCollectionView(APIView):
    ...
    def post(self, request):
        """Start live data collection for organization"""
        ...
        try:
            ...
            logger.info(f"Starting live data collection for {organization.name}, {days_back} days back")
            
            # Initialize live data service
            live_service = LiveDataService()
            
            # Collect all live data
            results = live_service.collect_all_live_data(organization, days_back)
    ...
    def get(self, request):
        ...
        try:
            ...
            live_service = LiveDataService()
            connection_status = live_service.get_account_connection_status(organization)
...
class TrendingContentView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            live_service = LiveDataService()
            trending_data = live_service._analyze_trending_content(organization, days_back)
...
class AccountConnectionStatusView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            live_service = LiveDataService()
            status_data = live_service.get_account_connection_status(organization)
    ...
    def get(self, request):
        ...
        try:
            ...
            try:
                ...
                account_count = SocialAccount.objects.filter(organization=request.user.organization).count()
                connected_count = SocialAccount.objects.filter(
                    organization=request.user.organization, 
                    status='connected'
                ).count()
    ...
    def get(self, request):
        try:
            ...
            analytics_query = SocialAnalytics.objects.filter(
                post_target__post__organization=request.organization,
